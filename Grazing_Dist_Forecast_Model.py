...
def run_precip_forecast_pipeline(prep_df, seq_length=48, forecast_start_year=2016, 
                               forecast_start_month=1, n_simulations=100,
    ...
    # Calculate n_future dynamically
    forecast_start_date = pd.Timestamp(f"{forecast_start_year}-{forecast_start_month:02d}-01")
    data_end_date = data['date'].max()
    
    # Calculate months between forecast start and end of data
    months_to_end = ((data_end_date.year - forecast_start_date.year) * 12 + 
                     (data_end_date.month - forecast_start_date.month))
    
    # Add 3 months as requested
    n_future = max(1, months_to_end + 3)  # Ensure at least 1 month forecast
    
    print(f"Forecast start: {forecast_start_date.strftime('%Y-%m')}")
    print(f"Data end: {data_end_date.strftime('%Y-%m')}")
    print(f"Months from forecast start to data end: {months_to_end}")
    print(f"n_future set to: {n_future} months")
    ...
    y_pred_mean_rescaled = np.maximum(y_pred_mean_rescaled, 0)
    ...
    def forecast_future(model, last_sequence, n_steps, scaler, feature_indices, data):
        ...
        # Ensure future predictions are non-negative
        future_pred_rescaled = np.maximum(future_pred_rescaled, 0)
    ...
    if len(future_pred) > 0:
        forecast_df = pd.DataFrame({
            ...
            'Lower_Bound_95': np.maximum(future_pred - 1.96 * future_std, 0),  # Ensure lower bound is non-negative
    ...
    # Return results (including n_future for reference)
    return {
        'model': model,  # Trained model for later use
        'scaler': scaler,  # Scaler for future predictions
        'n_future_used': n_future,  # The calculated n_future value
...
def GrazingDistForecaster(data_numeric, features, unique_ward1="Shapefile_wardName", 
                                   target_var="GrazingDist", seq_length=13, test_size=0.1, 
    ...
    #Derive default weights from forecast_weights if not provided
    ...
    def simple_data_preparation(data_numeric, features, target_var):
        ...
        """
        print("=== Simple Data Preparation (Original Approach + amountmilked) ===")
    ...
    # Ensure amountmilked is in features list if it exists in data (RETAINED FEATURE)
    if 'amountmilked' in data.columns and 'amountmilked' not in features:
        features = features + ['amountmilked']
        print(f"Added amountmilked to features. Updated features: {features}")
    elif 'amountmilked' in features and 'amountmilked' not in data.columns:
        print("Warning: amountmilked specified in features but not found in data columns")
        features = [f for f in features if f != 'amountmilked']
    ...
    # Store month to amountmilked mapping for forecasting (RETAINED)
    data_month_to_amountmilked = {}
    if 'amountmilked' in data.columns:
        for month in range(1, 13):
            month_data = data[data['month_num'] == month]
            if len(month_data) > 0:
                data_month_to_amountmilked[month] = month_data['amountmilked'].mean()
        print(f"amountmilked seasonal patterns created: {data_month_to_amountmilked}")
    
    print(f"Features being used: {features}")
    if 'amountmilked' in features:
        print(f"amountmilked feature included: {data['amountmilked'].describe() if 'amountmilked' in data.columns else 'Not found in data'}")
    
    print("Training ensemble components...")
    
    # =============================================================================
    # COMPONENT 1: LSTM MODEL 
    # =============================================================================
    
    def train_lstm_model():
        """
        Simplified LSTM training that matches the original model approach for consistency
        while retaining amountmilked support
        """
        print("Training LSTM model with simplified approach (original consistency + amountmilked)...")
        

        lstm_data = data[features].copy()
        print(f"Using features as provided: {features}")
        if 'amountmilked' in features:
            print(f"amountmilked range: {data['amountmilked'].min():.2f} to {data['amountmilked'].max():.2f}")
            print(f"amountmilked stats: mean={data['amountmilked'].mean():.2f}, std={data['amountmilked'].std():.2f}")
        ...
        return {
            ...
            'numeric_features': features,  # Use original features (including amountmilked)
    ...
    def train_var_model():
        print("Training VAR model with amountmilked support and significance filtering...")
        ...
        try:
            ...
            # Add amountmilked if available
            if 'amountmilked' in data.columns:
                var_features.append('amountmilked')
                print("Added amountmilked to VAR features")
    ...
    ensemble_metrics = {
        ...
        'amountmilked_included': 'amountmilked' in lstm_results['numeric_features'],
    ...
    print(f"amountmilked included: {ensemble_metrics['amountmilked_included']}")
    
    # =============================================================================
    # FIXED FUTURE FORECASTING - Only 3 forecasts from last actual data (with amountmilked)
    # =============================================================================
    ...
    def forecast_ensemble_fixed(n_future_window):
        ...
        Enhanced with amountmilked support (using original forecasting approach)
        """
        print(f"Generating ensemble forecasts with amountmilked (using {n_future_window}-step window, limiting to 3 forecasts)...")
        
        # LSTM forecasting (using original approach + amountmilked support)
        def forecast_lstm(last_sequence, n_steps):
            ...
            for step in range(n_steps):
                ...
                # Get amountmilked - use seasonal average if available (RETAINED FEATURE)
                curr_amountmilked = 0.0
                if 'amountmilked' in lstm_results['feature_indices']:
                    curr_amountmilked = data_month_to_amountmilked.get(curr_month, 0.0)
                ...
                future_amountmilked.append(curr_amountmilked)
                ...
                # Create new row for next prediction (original approach + amountmilked)
                try:
                    ...
                    if 'amountmilked' in lstm_results['feature_indices']:  # RETAINED FEATURE
                        temp_row[0, lstm_results['feature_indices']['amountmilked']] = curr_amountmilked
            ...
            return (lstm_pred_rescaled, lstm_std_rescaled, future_months, future_years,
                   future_seasons, future_precip, future_forecast_precip, future_gaps, future_amountmilked)
        
        # VAR forecasting (full window) with amountmilked support
        def forecast_var(n_steps):
            ...
            try:
                ...
                for step in range(n_steps):
                    ...
                    # Add amountmilked for VAR forecasts (RETAINED FEATURE)
                    curr_amountmilked = data_month_to_amountmilked.get(curr_month, 0.0)
                    future_amountmilked.append(curr_amountmilked)
        ...
        future_amountmilked = lstm_results_full[8]
        ...
        if last_actual_data is not None:
            ...
            if last_actual_idx is not None:
                ...
                future_amountmilked = future_amountmilked[last_actual_idx:end_idx]
                ...
                future_amountmilked = future_amountmilked[-4:]
        ...
        return (ensemble_future_pred, ensemble_future_std, lstm_future_pred, var_future_pred,
                ...
                future_forecast_precip, future_gaps, future_amountmilked)
    ...
    future_gaps, future_amountmilked = ensemble_results
    
    # =============================================================================
    # PREPARE OUTPUT DATAFRAMES (Same format as original + amountmilked)
    # =============================================================================
    ...
    test_results = pd.DataFrame({
        ...
        'Forecasted GrazingDist': ensemble_pred,
    ...
    # Add amountmilked to test results if available (RETAINED FEATURE)
    if 'amountmilked' in data.columns:
        test_results['amountmilked'] = data.iloc[test_indices]['amountmilked'].values
    
    # Future forecast results - LIMITED TO 4 ROWS (last actual + 3 forecasts) with amountmilked
    future_dates = [f"{year}-{month:02d}" for year, month in zip(future_years, future_months)]
    future_date_objects = [pd.Timestamp(f"{year}-{month:02d}-01") for year, month in zip(future_years, future_months)]
    ...
    forecast_results = pd.DataFrame({
        ...
        'Date': future_dates,
        'Date_Object': future_date_objects,
        'Forecasted GrazingDist': ensemble_future_pred,
    ...
    # Add amountmilked to forecast results (RETAINED FEATURE)
    if 'amountmilked' in lstm_results['numeric_features']:
        forecast_results['amountmilked'] = future_amountmilked
        ...
        if last_actual_row is not None:
            ...
            if last_actual_mask.any():
                ...
                if last_idx + 1 < len(forecast_results):
                    forecast_results.loc[last_idx, 'Month1_Forecast'] = forecast_results.loc[last_idx + 1, 'Forecasted GrazingDist']
                    
                if last_idx + 2 < len(forecast_results):
                    forecast_results.loc[last_idx, 'Month2_Forecast'] = forecast_results.loc[last_idx + 2, 'Forecasted GrazingDist']
                    
                if last_idx + 3 < len(forecast_results):
                    forecast_results.loc[last_idx, 'Month3_Forecast'] = forecast_results.loc[last_idx + 3, 'Forecasted GrazingDist']
