...
def process_residual_plots(countyId, Indicator):
    ...
    plt.axvline(mu, color='gray', linestyle='--', linewidth=2, alpha=0.7, label=f'Mean = {mu:.2f}')
    plt.axvline(0, color='red', linestyle='--', linewidth=2, alpha=0.7, label=f'Zero = {0:.2f}')
    ...
    plt.text(0.02, 0.95, f'Shapiro-Wilk p-value: {p_value:.4f}', transform=plt.gca().transAxes, bbox=dict(facecolor='white', alpha=0.8))
    plt.tight_layout()
    ...
    x_right_tail = np.linspace(mu + 0.95*std, x_max, 100)
    plt.fill_between(x_right_tail, normal_dist.pdf(x_right_tail), alpha=0.4, color='salmon')
    plt.title(f'Normality Density Plot for Milk Production 2-Month Forecast {target_column}\n(n={len(data)})', pad=15)
    plt.xlabel('Residual Value')
    plt.ylabel('Probability Density')
    plt.legend(loc='upper right')
    plt.grid(True, alpha=0.2, linestyle='--')
    ...
    x_right_tail = np.linspace(mu + 0.95*std, x_max, 100)
    plt.fill_between(x_right_tail, normal_dist.pdf(x_right_tail), alpha=0.4, color='salmon')
    plt.title(f'Normality Density Plot for Milk Production 3-Month Forecast {target_column}\n(n={len(data)})', pad=15)
    plt.xlabel('Residual Value')
    plt.ylabel('Probability Density')
    plt.legend(loc='upper right')
    plt.grid(True, alpha=0.2, linestyle='--')
