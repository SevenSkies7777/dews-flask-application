...
def process_outliers(countyId, dataCollectionExerciseId):
    ...
    df_milk = pd.read_sql(query, engine, params=(countyId, dataCollectionExerciseId))
    ...
    if df_milk.empty:
        ...
        if outliers_df.empty:
            ...
            outliers_df3['OutlierRunName'] = outliers_df3['CountyId'].astype(
                str) + '' + outliers_df3['ExerciseStartDate'].astype(str)
            outliers_df3['QuestionnaireType'] = "HHA"
            outliers_df4 = outliers_df3[
                ['OutlierRunName', 'DataCollectionExerciseId',
                 'QuestionnaireType']]
                    ...
                    with engine.begin() as conn:
                        result = conn.execute(
                            ...
                            {"run_name": outlier_run_names[i],
                             "exercise_id": int(
                                 data_collection_exercise_ids[i])}
                        )
                        print(
                            f"Deleted {result.rowcount} previous outlier runs successfully")
            ...
            data_collection_exercise_id = \
                outliers_df3['DataCollectionExerciseId'].iloc[0]
            ...
            outliers_df6 = outliers_df5[
                ['OutlierRunId', 'WardId', 'HouseHoldId', 'analyzed_column',
                 'IndicatorType', 'outlier_type', 'OutlierValue',
            ...
            for _, row in outliers_df6.iterrows():
                ...
                for col in outliers_df6.columns:
                    ...
                    elif hasattr(value, 'dtype') and np.issubdtype(value.dtype,
                                                                   np.integer):
    ...
    coping_df = pd.read_sql(query, engine, params=(countyId, dataCollectionExerciseId))
    ...
    crop_df = pd.read_sql(query, engine, params=(countyId, dataCollectionExerciseId))
    ...
    if coping_df.empty:
        ...
        # Call the method with your dataframe
        results = hha_outliers.detect_outliers_crop_production(crop_df=crop_df)
    ...
    crop_df = pd.read_sql(query, engine,
                          params=(countyId, dataCollectionExerciseId))
    ...
    livestock_df = pd.read_sql(query, engine, params=(countyId, dataCollectionExerciseId))
    ...
    food_c_df = pd.read_sql(query, engine, params=(countyId, dataCollectionExerciseId))
